  ...
  return <DashboardLayout eyebrow="SURA / AI STUDIO" title="Turn the picture into a way forward." description="Use an optional room or personal image, a real budget, and the direction you want to reach. SURA returns a private, structured plan—not a promise.">
    <div className="grid gap-7 xl:grid-cols-[0.92fr_1.08fr]">
      ...
      <section className="rounded-[1.6rem] border border-[var(--sura-border)] bg-[var(--sura-paper)] p-6 sm:p-8">
        ...
        <div className="mt-7 grid max-h-72 gap-3 overflow-y-auto pr-1 sm:grid-cols-2">{journeys.map((journey) => <button key={journey.value} onClick={() => setKind(journey.value)} className={`vb-focus rounded-2xl border p-4 text-left ${kind === journey.value ? "border-[var(--sura-primary)] bg-[var(--sura-primary)] text-[var(--sura-paper)]" : "border-[var(--sura-border)] bg-white text-[var(--sura-ink)]"}`}><p className="font-bold">{journey.label}</p><p className={`mt-1 text-xs leading-5 ${kind === journey.value ? "text-white/75" : "text-[#776957]"}`}>{journey.copy}</p></button>)}</div>
        <div className="mt-6 grid gap-5 sm:grid-cols-[1fr_10rem]">
          <label className="block"><span className="text-xs font-bold uppercase tracking-[0.08em] text-[#725f4c]">What are you trying to achieve?</span><textarea id="sura-ai-brief" value={brief} onChange={(event) => setBrief(event.target.value)} className="vb-focus mt-2 min-h-28 w-full rounded-xl border border-[var(--sura-border)] bg-white p-3 text-sm" placeholder="For example: I want a small living room that feels warm, social, and not crowded under KES 25,000." /></label>
