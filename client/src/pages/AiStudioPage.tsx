  ...
  return <DashboardLayout eyebrow="SURA / AI STUDIO" title="Turn the picture into a way forward." description="Use an optional room or personal image, a real budget, and the direction you want to reach. SURA returns a private, structured plan—not a promise.">
    <div className="grid gap-7 xl:grid-cols-[0.92fr_1.08fr]">
      <aside className="rounded-[1.6rem] border border-[var(--sura-border)] bg-[var(--sura-paper)] p-6">
        ...
        <div className="mt-6 rounded-2xl bg-[var(--sura-primary)] p-5 text-[var(--sura-paper)]"><p className="vb-kicker text-[var(--sura-accent)]">Current aesthetic</p><p className="vb-serif mt-2 text-3xl">{aesthetic}</p><p className="mt-3 text-sm leading-6 text-white/75">Your palette frames this direction. Your saved mix gives the AI a creative lens; your written goal remains in charge.</p>{savedAestheticMix.isLoading && <p role="status" className="mt-4 rounded-xl border border-white/15 bg-white/10 p-3 text-xs leading-5 text-white/80">Opening your saved aesthetic mix. Your current palette remains available.</p>}{savedAestheticMix.isError && <div role="alert" className="mt-4 rounded-xl border border-white/15 bg-white/10 p-3 text-xs leading-5 text-white/80"><p>Your saved mix could not be opened. Your current palette remains available for this request.</p><button onClick={() => savedAestheticMix.refetch()} className="vb-focus mt-2 rounded-full border border-white/30 px-3 py-1.5 text-[0.68rem] font-bold">Retry saved mix</button></div>}<div className="mt-4 flex flex-wrap gap-1.5">{aestheticMix.map((name) => <span key={name} className="rounded-full border border-white/15 bg-white/10 px-2.5 py-1 text-[0.68rem] font-bold">{name}</span>)}</div></div>
      </aside>
    ...
    {assist.data && <section className="mt-7 overflow-hidden rounded-[1.8rem] border border-[var(--sura-border)] bg-[var(--sura-paper)]"><div className="grid lg:grid-cols-[0.85fr_1.15fr]"><div className="min-h-80 bg-[var(--sura-primary)] p-6 text-[var(--sura-paper)]"><p className="vb-kicker text-[var(--sura-accent)]">AI concept · not a guaranteed outcome</p><img src={assist.data.generatedImageUrl} alt="AI-generated concept for the requested aesthetic direction" className="mt-5 h-64 w-full rounded-2xl object-cover" /></div><div className="p-7 sm:p-9"><p className="vb-kicker text-[var(--sura-accent)]">Your private plan</p><h2 className="vb-serif mt-3 text-4xl">{assist.data.plan.title}</h2><p className="mt-4 text-sm leading-7 text-[#706252]">{assist.data.plan.designDirection}</p><div className="mt-6 grid gap-5 sm:grid-cols-2"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[#725f4c]">What to prioritise</p><ul className="mt-3 space-y-2 text-sm">{assist.data.plan.priorities.map((priority) => <li key={priority} className="flex gap-2"><Check className="mt-0.5 h-4 w-4 text-[var(--sura-accent)]" />{priority}</li>)}</ul></div><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[#725f4c]">Product categories to explore</p><div className="mt-3 flex flex-wrap gap-2">{assist.data.plan.recommendedCategories.map((category) => <span key={category} className="rounded-full border border-[var(--sura-border)] px-3 py-2 text-xs font-bold">{category}</span>)}</div></div></div><p className="mt-6 rounded-xl bg-[var(--sura-muted)] p-4 text-sm leading-6">{assist.data.plan.shoppingLens}</p><p className="mt-3 text-xs leading-5 text-[#756655]">{assist.data.plan.safetyNote}</p><Link href="/shop" className="vb-button vb-focus mt-6 inline-flex rounded-full bg-[var(--sura-primary)] px-5 py-3 text-sm font-bold text-[var(--sura-paper)]">Explore connected company products</Link></div></div><div className="border-t border-[var(--sura-border)] p-7 sm:p-9"><p className="vb-kicker text-[var(--sura-accent)]">Connected product signals · only when a verified listing exists</p>{assist.data.connectedProducts.length ? <div className="mt-5 grid gap-4 lg:grid-cols-3">{assist.data.connectedProducts.map((quote) => <article key={quote.product.id} className="rounded-2xl border border-[var(--sura-border)] bg-[var(--sura-page)] p-4"><p className="font-bold text-[var(--sura-primary)]">{quote.product.name}</p><p className="mt-1 text-xs text-[#756655]">{quote.company.name} · {quote.company.city}</p><dl className="mt-4 space-y-2 text-xs leading-5"><div className="flex justify-between gap-3"><dt>Item price</dt><dd className="font-bold">KES {quote.merchandiseSubtotalKes.toLocaleString()}</dd></div><div className="flex justify-between gap-3"><dt>Delivery estimate</dt><dd className="font-bold">KES {quote.deliveryKes.toLocaleString()}</dd></div><div className="flex justify-between gap-3"><dt>SURA commission</dt><dd className="font-bold">KES {quote.commissionKes.toLocaleString()} · {quote.commissionRatePct}%</dd></div><div className="flex justify-between gap-3"><dt>Seller settlement</dt><dd className="font-bold">KES {quote.sellerSettlementKes.toLocaleString()}</dd></div><div className="flex justify-between gap-3 border-t border-[var(--sura-border)] pt-2"><dt>Indicative total</dt><dd className="font-bold">KES {quote.customerTotalKes.toLocaleString()}</dd></div></dl></article>)}</div> : <p className="mt-4 rounded-xl border border-dashed border-[var(--sura-border)] bg-[var(--sura-page)] p-4 text-sm leading-6 text-[#756655]">No verified connected listing matches this private direction in {selectedCity} yet. SURA will not invent inventory, price, delivery, or availability.</p>}<p className="mt-4 text-xs leading-5 text-[#756655]">These are transparent planning estimates, not reservation, stock, or delivery guarantees. A live payment remains unavailable until a verified provider is configured.</p></div></section>}
  </DashboardLayout>;
}
