...
export function LocationPicker({ compact = false }: { compact?: boolean }) {
  ...
  return <div className="relative">
    ...
    {isOpen && <div data-testid="county-finder-panel" className={`absolute right-0 top-[calc(100%+0.65rem)] z-50 overflow-hidden rounded-[1.25rem] border border-[#d8cbb8] bg-[#fffdf9] shadow-[0_20px_48px_rgba(54,38,22,0.18)] ${compact ? "w-[min(21rem,calc(100vw-1.5rem))] max-xl:fixed max-xl:inset-x-2 max-xl:top-[4.75rem] max-xl:max-h-[calc(100dvh-5.5rem)] max-xl:w-auto max-xl:overflow-y-auto" : "w-[22rem]"}`}>
