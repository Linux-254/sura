...
export function SuraPageSkeleton({ cards = 3, dashboard = false }: { cards?: number; dashboard?: boolean }) {
  return <div className="animate-in fade-in duration-300"><div className="max-w-2xl"><SuraShimmer className="h-3 w-28" /><SuraShimmer className="mt-5 h-12 w-[85%]" /><SuraShimmer className="mt-4 h-4 w-[70%]" /></div><div className={`mt-10 grid gap-5 ${dashboard ? "lg:grid-cols-2" : "sm:grid-cols-2 lg:grid-cols-3"}`}>{Array.from({ length: cards }).map((_, index) => <div key={index} className="overflow-hidden rounded-[1.6rem] border border-[var(--sura-border)] bg-[var(--sura-paper)] p-5"><SuraShimmer className="h-40 w-full" /><SuraShimmer className="mt-5 h-4 w-20" /><SuraShimmer className="mt-3 h-8 w-[78%]" /><SuraShimmer className="mt-4 h-3 w-full" /><SuraShimmer className="mt-2 h-3 w-[72%]" /></div>)}</div></div>;
}
