...
export function ProductGallery({ images, name, compact = false }: ProductGalleryProps) {
  ...
  return <div className="space-y-3">
    <div className={`group relative overflow-hidden rounded-2xl bg-[#eee5d8] ${compact ? "aspect-[4/3]" : "min-h-[25rem] sm:min-h-[34rem]"}`}>
      <img src={activeImage} alt={`${name} product image ${activeIndex + 1} of ${safeImages.length}`} className="h-full w-full object-cover transition-opacity duration-300" />
    ...
    {safeImages.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1" aria-label={`${name} image gallery`}>{safeImages.map((image, index) => <button key={`${image}-${index}`} onClick={() => setActiveIndex(index)} className={`vb-focus relative h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 bg-[#eee5d8] ${activeIndex === index ? "border-[#a66231]" : "border-transparent opacity-65 hover:opacity-100"}`} aria-label={`View ${name} image ${index + 1}`} aria-pressed={activeIndex === index}><img src={image} alt="" className="h-full w-full object-cover" /></button>)}</div>}
  </div>;
}
